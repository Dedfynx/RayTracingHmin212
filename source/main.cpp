...
bool shadowFeeler(vec4 p0, Object *object)
{
  ...
  vec4 vLumiere = normalize(lightPosition - p0);
  vLumiere.w = 0.0;

  double distLum = sqrt(dot(vLumiere,vLumiere));
  double dist = object->intersect(p0, vLumiere).t;

  if (dist < distLum - EPSILON)
  {
    inShadow = true;
  }
...
vec4 castRay(vec4 p0, vec4 E, Object *lastHitObject, int depth)
{
  ...
  double distLastHitObject;
  Object::IntersectionValues infoLastHit;

  for (int i = 0; i < sceneObjects.size(); i++)
  {
    double dist;
    Object::IntersectionValues iv = sceneObjects[i]->intersect(p0, E);
    if ((dist = iv.t) != infini)
    {
      if (lastHitObject != NULL)
      {
        if (dist < distLastHitObject && dist > EPSILON)
        {
          lastHitObject = sceneObjects[i];
          distLastHitObject = dist;
          infoLastHit = iv;
        }
      }
      else
  ...
  if (lastHitObject != NULL)
  {
    if (lastHitObject->name == "Mirrored Sphere")
    {
      color = castRay(infoLastHit.P, infoLastHit.N, NULL, 0);
    }
    else if (lastHitObject->name == "Glass sphere")
    {
      //refraction
      float omega1 = dot(E, infoLastHit.N);
      if (omega1 < -1)
      {
        omega1 = -1;
      }
      ...
      float ir1, ir2; //indice refraction

      ir1 = 1;
      ir2 = lastHitObject->shadingValues.Kr;
      vec4 n = infoLastHit.N; //normale
      if (omega1 < 0)
      {
      ...
      {
        //on est dans la sphere, on echange les indices de refractions
        std::swap(ir1, ir2);
        n = -infoLastHit.N;
      }
      float iR = ir1 / ir2;
      float k = 1 - iR * iR * (1 - omega1 * omega1);
      vec4 T;
      if (k < 0)
      {
        T = 0; //pas de refraction
      }
      else
      {
        T = iR * E + (iR * omega1 - std::sqrt(k)) * n;
      }
      color = castRay(infoLastHit.P, T, NULL, 0);
    }
    else
    {
      //normale du point d'intersection
      vec4 N = infoLastHit.N; 
      N.w = 0.0;
      //vecteur directeur du rayon lumineux
      vec4 L = normalize(lightPosition - infoLastHit.P);
      L.w = 0.0;
      //vecteur directeur de la vue
      vec4 V = normalize(p0 - infoLastHit.P);
      V.w = 0.0;
      
      vec4 R = normalize(2.0 * dot(N, L) * N - L);
      R.w = 0.0;

    //angle entre la normale et la vecteur de lumiere
      ...
      for (int i = 0; i < sceneObjects.size(); i++)
      {
        if (shadowFeeler(infoLastHit.P, sceneObjects[i]))
        {
          eclairage = false; //est dans l'ombre
          break;//plus besoins de chercher d'obstacles
        }
      }

      color4 material_ambient(lastHitObject->shadingValues.color * lastHitObject->shadingValues.Ka);
      color4 material_diffuse(lastHitObject->shadingValues.color * lastHitObject->shadingValues.Kd);
      color4 material_specular(lastHitObject->shadingValues.Ks * lightColor);

      color4 colorAmbient = (material_ambient * GLState::light_ambient);
      colorAmbient[3] = 1.0;
      color4 colorDiffuse = (material_diffuse * GLState::light_diffuse * theta * eclairage);
      colorDiffuse[3] = 1.0;
      color4 colorSpecular = (material_specular * GLState::light_specular * pow(alpha, lastHitObject->shadingValues.Kn));
      colorSpecular[3] = 1.0;
...
void initCornellBox()
{
  ...
  lightColor = color4(1.0, 1.0, 1.0, 0.5);
  ...
  { //Back Wall
    ...
    _shadingValues.color = vec4(0.6, 0.8, 0.1, 1.0);
  ...
  { //Ceiling
    ...
    Object::ShadingValues _shadingValues;
    _shadingValues.color = vec4(0.1, 0.3, 1.0, 1.0);
    _shadingValues.Ka = 0.0;
    _shadingValues.Kd = 1.0;
    _shadingValues.Ks = 0.0;
    _shadingValues.Kn = 16.0;
...

  {
    ...
    _shadingValues.Ka = 0.0;  //coef lumiere ambiante
    _shadingValues.Kd = 0.0;  //coef lumiere diffuse
    _shadingValues.Ks = 1.0;  //coef lumiere spéculaire (phong)
    _shadingValues.Kn = 16.0; //le n dans phong ?
    _shadingValues.Kt = 0.0;
    _shadingValues.Kr = 0.0;
    sceneObjects[sceneObjects.size() - 1]->setShadingValues(_shadingValues);
    sceneObjects[sceneObjects.size() - 1]->setModelView(mat4());
...
void initUnitSphere()
{
  ...
  {
    sceneObjects.push_back(new Sphere("Diffuse sphere", vec3(1.0, 0.0, 0.0), 1.0));
    Object::ShadingValues _shadingValues;
    _shadingValues.color = vec4(1.0, 0.0, 0.0, 1.0);
    _shadingValues.Ka = 0.0;
    _shadingValues.Kd = 1.0;
  ...
  {
    sceneObjects.push_back(new Sphere("Diffuse sphere2", vec3(-1.0, 0.0, 0.0), 1.0));
    Object::ShadingValues _shadingValues;
    _shadingValues.color = vec4(0.0, 1.0, 0.0, 1.0);
    _shadingValues.Ka = 0.0;
    _shadingValues.Kd = 1.0;
